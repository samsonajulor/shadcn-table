...
export async function getTasks(input: GetTasksSchema): Promise<{
  ...
  try {
    ...
    return { data: [{
        ...
        label: 'bug',
        priority: 'low',
        createdAt: '12/02/24' as unknown as Date,
        updatedAt: '12/02/24' as unknown as Date,
    }], pageCount: 4 }
  } catch (err) {
...
export async function getTaskCountByStatus(): Promise<{
    status: 'todo' | 'in-progress' | 'done' | 'canceled';
    count: number;
}[]> {
  noStore()
  try {
    ...
    //   .execute()
    return [{ status: 'todo', count: 2 }]
  } catch (err) {
    return []
  }
}

export async function getTaskCountByPriority(): Promise<{
